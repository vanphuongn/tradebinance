...
const findRetestFutureForBuy = async (priceDatas, coinName2, timeRequest) => {
    ...
    try {
        ...
        for (var i = 0; i < ema10.length; i++) {
            if ((ema10[ema10.length - (i + 1)] < kc_out.upper[kc_out.upper.length - (i + 1)])
                && (ema10[ema10.length - i] > kc_out.upper[kc_out.upper.length - i])
                //&& (ema10[ema10.length - i] > ema89[ema89.length - i])
                && (lastestEma10UnderEma50 > lastestEma10UnderEma89)
            ) {
                if (i < 15) {
                    console.log(timeRequest + " BUY KC " + coinName2 + "   : " + i)
                }
                if (i < 5) {
                    bot.sendMessage(chatId, timeRequest + " BUY KC " + coinName2 + "   : " + i)
                }
            }

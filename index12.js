...
const findRetestFutureForBuy = async (coinName2, timeRequest) => {
                ...
                if ((lastest4EmaIsAscendingOrder != -1) && (lastestEma10UnderEma89 != -1)) {
                    ...
                    try {
                        ...
                        if (hasLowerEma89 == true
                            && (macdData2[macdData2.length - 1 - candleHasLowerEma89Idx].MACD < macdData2[macdData2.length - 1 - candleHasLowerEma89Idx].signal)
                            ...
                            //  console.log(coinName2 + " highestPrice   " + highestPrice + " lastest4EmaIsAscendingOrderprice "
                            //         +priceDatas[priceDatas.length - 1 - lastest4EmaIsAscendingOrder].close)

                            if ((macdData2[macdData2.length - 1 - lastest4EmaIsAscendingOrder].MACD > macdData2[macdData2.length - 1 - lastest4EmaIsAscendingOrder].signal)
                                && (priceDatas[priceDatas.length - 1 - lastest4EmaIsAscendingOrder].low < highestPrice)
                            ) {
                                //   if(lastest4EmaIsAscendingOrder < 15)
                                {
                                    console.log(coinName2 + "  " + timeRequest + "  lastest4EmaIsAscendingOrder   " + lastest4EmaIsAscendingOrder + " lastest4EmaIsAscendingOrderprice "
                                        + priceDatas[priceDatas.length - 1 - lastest4EmaIsAscendingOrder].close

                                        + "  highestPrice " + highestPrice)

                                    if (lastest4EmaIsAscendingOrder < 4) {
                                        bot.sendMessage(chatId, coinName2 + "  " + timeRequest + "  buy for retest  " + lastest4EmaIsAscendingOrder + " p " + priceDatas[priceDatas.length - 1 - lastest4EmaIsAscendingOrder].close)
                                        bot.sendMessage(chatId, coinName2 + "_" + timeRequest + "_" + "buy")
                                    }
                                }
                            }
        ...
        if ((ema10[ema10.length - 1] < ema20[ema20.length - 1])
            ...
            try {
                if ((lastest4EmaIsAscendingOrder != -1) && (lastestEma10OverEma89 != -1)) {
                    ...
                    //    
                    if ((hasHigherEma89 == true) && (priceDatas[priceDatas.length - 1 - lastest4EmaIsAscendingOrder].low < priceDatas[priceDatas.length - 1 - candleHasHigherEma89Idx].high)) {
                        var idexForShell = -1
                        ...
                        //         bot.sendMessage(chatId, coinName2 + "  " + timeRequest + "  sell by macd  " + idexForShellCheckByMacd + " p " + priceDatas[priceDatas.length - 1 - idexForShellCheckByMacd].close)
                        //     }
                        // }
                        if (idexForShell > 0) {
                            var lastEma10OverCutEma20 = -1
                            for (var i = idexForShell; i < lastestEma10OverEma89; i++) {
                                if ((ema10[ema10.length - 1 - i] > ema20[ema20.length - 1 - i]) && (ema10[ema10.length - 1 - (i + 1)] < ema20[ema20.length - 1 - (i + 1)])) {
                                    lastEma10OverCutEma20 = i;
                                }
                            }
                            if (lastEma10OverCutEma20 >= 0) {
                                for (var i = lastEma10OverCutEma20; i < idexForShell; i++) {
                                    if (priceDatas[priceDatas.length - 1 - i].high > ema89[ema89.length - 1 - i]) {
                                        //   bot.sendMessage(chatId, coinName2 + "  " + timeRequest + "  sell ema  " + idexForShell + " p " + priceDatas[priceDatas.length - 1 - idexForShell].close)
                                        //    bot.sendMessage(chatId, coinName2 + "_" + timeRequest + "_" + "sell")
                                        console.log(coinName2 + "  " + timeRequest + "  sell ema " + idexForShell + " p " + priceDatas[priceDatas.length - 1 - idexForShell].close)
                                    }
                                }
                            }
                            // console.log(coinName2 + "  " + timeRequest + "  sell  " + idexForShell + " p " + priceDatas[priceDatas.length - 1 - idexForShell].close)
                        ...
                        if ((idexForShell < 4) && (idexForShell > 0)) {
                            ...
                            if (lastEma10OverCutEma20 >= 0) {
                                var hasCandlerOverEma89 = false;
                                var idx = -1
                                for (var i = lastEma10OverCutEma20; i < idexForShell; i++) {
                                    if (priceDatas[priceDatas.length - 1 - i].high > ema89[ema89.length - 1 - i]) {
                                        hasCandlerOverEma89 = true;
                                        idx = i;
                                    }
                                }

                                if (hasCandlerOverEma89 == true) {
                                    bot.sendMessage(chatId, coinName2 + "  " + timeRequest + "  sell ema  " + idx + " p " + priceDatas[priceDatas.length - 1 - idx].close)
                                    bot.sendMessage(chatId, coinName2 + "_" + timeRequest + "_" + "sell")
                                }
                            }
...
const updatePrice = async (timeRequest) => {
    ...
    try {
        ...
        // for(var coinIndex = 0; coinIndex < top20.length; coinIndex++)
        {
            ...
            //var coinName2 = "RIFUSDT"

            if (coinName2.includes("USDT") && (coinName2 != "COCOSUSDT") && (coinName2 != "BICOUSDT")) {
                try {

                    var test15m = await findRetestFutureForBuy(coinName2, timeRequest)
                    await wait(200);

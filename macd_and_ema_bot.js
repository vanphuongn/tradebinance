...
const updatePriceForBuy =async (coinName2,timeRequest)=>{
        try{
            timeRequest = "15m"
                   ...
                   let price15mDatas =   await client.candles({ symbol: coinName2, limit:1000,interval:"15m" })
                   var prices15m = []
                   for(var i =0; i < price15mDatas.length; i++)
                   {
                      //  console.log(coinName2+ "   "+i + "    priceDatas " + priceDatas[i].close)
                      prices15m.push(Number(price15mDatas[i].close))
                   }
                   var macd15mInput = {
                    values            : prices15m,
                    fastPeriod        : 12,
                    ...
                    var macd26_15m_Data = MACD.calculate(macd15mInput)
                ...
                //	    console.log(i + "    priceDatas " + priceDatas[i].close)
                    ...
                    var macd26Data = MACD.calculate(macd26Input)
                    var macd89Data = MACD.calculate(macd89Input)
    //                   var result =  MACD.calculate(macdInput);
               //   console.log("macdInput :"+JSON.stringify(macdInput))
                ...
                if(
                 ...
                 //  && 
                   ...
                   )
                   {
                        ...
                        if(macd89Data[(macd89Data.length -1)].MACD  / priceDatas[priceDatas.length -1].close < (-1.5/100))
                        {
                            // check last 30 candle


                            
                            if((ema10[ema10.length-1] > ema20[ema20.length-1]) &&(ema10[ema10.length-2] < ema20[ema20.length-2]) )
                            {
                                console.log("COin out range " + coinName2 + "  macd15m  " + macd26_15m_Data[(macd26_15m_Data.length -1)].MACD)
                            }

                    //    //     console.log("COin out range " + coinName2 + "  macd15m  " + macd26_15m_Data[(macd26_15m_Data.length -1)].MACD)
                            // if( (macd26Data[(macd26Data.length -1)].MACD > macd26Data[(macd26Data.length -1)].signal)
                            //     && (macd26Data[(macd26Data.length -2)].MACD < macd26Data[(macd26Data.length -2)].signal)
                            //     )
                            //     {
                            //         console.log("COin out range " + coinName2 + "  macd15m  " + macd26Data[(macd26Data.length -1)].MACD)
